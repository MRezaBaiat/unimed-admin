...
function Day (props) {
  ...
  if (
    ...
    if (Array.isArray(minRangeDuration)) {
      const i = minRangeDuration.findIndex(i =>
  ...
  if (
    ...
    if (Array.isArray(maxRangeDuration)) {
      const i = maxRangeDuration.findIndex(i =>
        jMoment(i.date).isSame(jMoment(selectedStartDate, 'day'))
      );
      if (
        i >= 0 &&
  ...
  if (!dateOutOfRange) {
    ...
    if (
      ...
      if (isToday) {
        selectedDayColorStyle = { ...selectedDayColorStyle, ...todayTextStyle };
      }
    ...
    if (
      workHours && workHours.length !== 0 && !isThisDaySameAsSelectedStart && _isDayBeforeToDay()) {
      daySelectedStyle = styles.selectedDays;
      selectedDayColorStyle = styles.selectedDaysLabel;
    }
    const _touchable = () => {
      if (
        workHours && workHours.length !== 0 && _isDayBeforeToDay()
      ) {
        return false;
      } else {
        return true;
    ...
    return (
      <div style={{ ...styles.dayWrapper, ...customContainerStyle }}>
        <div
          disabled={_touchable()}
          style={{ ...customDateStyle, ...daySelectedStyle, ...propSelectedDayStyle }}
          onClick={() => !_touchable() && onPressDay(day)}
        >
          <AppTextView
            text={day}
            style={{
    ...
    return (
      <div style={styles.dayWrapper}>
        <AppTextView text={day} style={{ ...textStyle, ...styles.disabledText }}/>
      </div>
    );
  }
}
